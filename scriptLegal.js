let test;
let start = Math.floor(Math.random() * 260);
var firstWord;
...
function loadXMLDoc() {
    ...
    xhttp.open("GET", "legal_english.json", true);
    xhttp.send();
  }
  function word(a) {
  ...
  var counter = Math.floor(Math.random() * 260);
  var wordsInTest = 20;
  var words = 1;
  var score = 0;
  ...
  function next() {
    ...
    if(words == wordsInTest) {
      ...
      counter = Math.floor(Math.random() * 260);;
      words += 1;
      //console.log(counter);
